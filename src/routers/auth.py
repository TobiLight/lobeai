...
@auth_router.post("/login", summary="User login",
                  response_model=Union[UserProfile, dict],
                  response_model_exclude_none=True)
async def login(request: Request):
    ...
    if id_info["iss"] == "https://accounts.google.com":
        existing_user = await db.user.\
            find_unique(where={"email": id_info["email"]})
        if not existing_user:
            try:
                new_user = await db.user.create({
                    ...
                    "email": id_info["email"],
                    "name": id_info["name"],
                }, include={"database_connections": True,
                            "conversations": True})
                ...
                update_conversation = await db.conversation.\
                    update(where={"id": default_conversation.id},
                ...
                access_token = jwt.encode(
                    ...
                    getenv("JWT_SECRET_KEY"), getenv("ALGORITHM"))
